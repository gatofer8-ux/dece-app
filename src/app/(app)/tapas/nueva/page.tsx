...
import { createTapasSession } from "../actions";

export default async function NuevaTapasSessionPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Nueva aplicación del juego de arquetipos"
        description="Se genera un código y un enlace para que los estudiantes del curso jueguen."
      />
      <form action={createTapasSession} className="card p-6 space-y-5 max-w-2xl">
        <div>
          <label className="label text-xs">Título / referencia</label>
          <input name="title" defaultValue="Juego de arquetipos TaPas" className="input text-sm" required />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div>
            <label className="label text-xs">Curso</label>
            <input name="course" list="tapas-courses" className="input text-sm" placeholder="Ej. 2º BGU" />
            <datalist id="tapas-courses">{courses.map((c) => <option key={c} value={c} />)}</datalist>
          </div>
          <div>
            <label className="label text-xs">Paralelo</label>
            <input name="parallel" list="tapas-parallels" className="input text-sm" placeholder="Ej. A" />
            <datalist id="tapas-parallels">{parallels.map((p) => <option key={p} value={p} />)}</datalist>
          </div>
          <div>
        ...
        <p className="text-[11px] text-slate-400">
          Si defines curso y paralelo, el estudiante podrá elegir su nombre de la lista. Si los dejas en blanco, escribirá sus datos.
        </p>
