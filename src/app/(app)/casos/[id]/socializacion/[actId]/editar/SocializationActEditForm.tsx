...
export default function SocializationActEditForm({
  ...
  const [preparedByName, setPreparedByName] = useState(act.prepared_by_name || defaultPreparedBy || "");
  const [approvedByName, setApprovedByName] = useState(act.approved_by_name || defaultApprovedBy || "");
  const [receivedByName, setReceivedByName] = useState(act.received_by_name || "");
  const [receivedByRole, setReceivedByRole] = useState(act.received_by_role || "Tutor del curso");

  const [deceSig, setDeceSig] = useState<DualSignatureData | null>(parsedSigs.dece || null);
  const [approvedSig, setApprovedSig] = useState<DualSignatureData | null>(parsedSigs.approved || null);
  const [receivedSig, setReceivedSig] = useState<DualSignatureData | null>(parsedSigs.received || null);
